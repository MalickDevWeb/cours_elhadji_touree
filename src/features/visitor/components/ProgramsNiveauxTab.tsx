...
const getSubjectIcon = (subjectName: string) => {
  ...
  if (lower.includes('fran')) return BookText;
  if (lower.includes('physi') || lower.includes('chim')) return FlaskConical;
  if (lower.includes('svt') || lower.includes('bio')) return Dna;
  if (lower.includes('angla')) return Globe;
  if (lower.includes('philo')) return Brain;
  return BookOpen;
};
              ...
              <div key={cyc.id} className="p-4 bg-slate-50/70 hover:bg-slate-50 rounded-2xl border border-slate-200/70 space-y-3 transition duration-200">
                <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200/60 pb-2.5">
                  ...
                  <div className="flex flex-wrap items-center gap-1.5">
                    <span className="bg-amber-500/10 text-amber-800 border border-amber-200/80 text-[10.5px] font-bold px-2.5 py-1 rounded-xl">
                      Inscription : <strong className="font-black text-amber-900">{cyc.registrationFee.toLocaleString('fr-FR')} F</strong>
                    </span>
                    <span className="bg-emerald-500/10 text-emerald-800 border border-emerald-200/80 text-[10.5px] font-bold px-2.5 py-1 rounded-xl">
                      <strong className="font-black text-emerald-900">{cyc.monthlyFee.toLocaleString('fr-FR')} F</strong>/mois
                    </span>
                  </div>
                </div>
                <div className="flex items-center gap-2 text-[11px]">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {cyc.levels.map((lvl) => (
                      <span key={lvl} className="bg-white text-slate-800 px-2 py-0.5 rounded-lg border border-slate-200 shadow-2xs font-semibold text-[10.5px]">
                        {lvl}
                      </span>
                    ))}
                  </div>
            ...
            {subjects.filter(s => s.active).map(sub => {
              ...
              return (
                <div key={sub.id} className="flex items-center gap-2 px-3 py-2.5 bg-slate-50/80 hover:bg-slate-100/80 border border-slate-200/60 rounded-xl text-xs font-bold text-slate-800 transition">
                  <SubIcon className="w-4 h-4 text-sky-600 shrink-0" />
