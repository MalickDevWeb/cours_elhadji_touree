...
export function AddAssignmentForm({ students, teachers, subjects, groups, assignments = [], onAddAssignment, onClose }: AddAssignmentFormProps) {
  const [f, setF] = useState({ type: 'INDIVIDUEL' as 'INDIVIDUEL' | 'GROUPE', studentId: '', groupId: '', teacherId: '', subjectId: '', schedule: '', location: '' });

  const conflicts = useMemo(() => {
    const existing = assignments.map(a => ({
    ...
    return findPlanningConflicts({
      teacherId: f.teacherId, roomOrLocation: f.location, schedule: f.schedule,
      title: f.type === 'INDIVIDUEL' ? 'Nouveau cours individuel' : 'Nouveau cours groupe'
    }, existing);
  }, [f.teacherId, f.location, f.schedule, assignments, f.type]);
  ...
  return (
    <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl max-w-lg w-full text-xs animate-in fade-in zoom-in-95 duration-200 my-auto sm:my-0">
      ...
      <form onSubmit={handleSaveAsg} className="p-5 space-y-3.5">
        ...
        {f.type === 'INDIVIDUEL' ? (
          <div><label className={labelCls}>Élève</label><SearchableSelect value={f.studentId} onChange={v => setF({ ...f, studentId: v })} options={students.map(s => ({ value: s.id, label: `${s.firstName} ${s.lastName}` }))} placeholder="Sélectionner l'élève" className={selectCls} required /></div>
        ) : (
          <div><label className={labelCls}>Groupe / Salle</label><SearchableSelect value={f.groupId} onChange={v => setF({ ...f, groupId: v })} options={groups.map(g => ({ value: g.id, label: `${g.name} (${g.room})` }))} placeholder="Sélectionner le groupe" className={selectCls} required /></div>
        )}

        <div className="grid grid-cols-2 gap-3">
          <div><label className={labelCls}>Professeur</label><SearchableSelect value={f.teacherId} onChange={v => setF({ ...f, teacherId: v })} options={teachers.map(t => ({ value: t.id, label: t.fullName }))} placeholder="Professeur" className={selectCls} required /></div>
          <div><label className={labelCls}>Matière</label><SearchableSelect value={f.subjectId} onChange={v => setF({ ...f, subjectId: v })} options={subjects.map(s => ({ value: s.id, label: s.name }))} placeholder="Matière" className={selectCls} required /></div>
        </div>

        <div><label className={labelCls}>Salle / Lieu</label><input type="text" placeholder="ex: Salle Einstein ou Domicile" value={f.location} onChange={e => setF({ ...f, location: e.target.value })} className={inputCls} required /></div>
