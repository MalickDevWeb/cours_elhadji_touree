    ...
    try {
      ...
      const htmlContent = `
        <div style="font-family: sans-serif; padding: 20px; color: #1e293b; background-color: #f8fafc; border-radius: 12px;">
          <h2 style="color: #0284c7; margin-top: 0;">${(settings as any).name || 'Centre de Soutien Scolaire'}</h2>
          <p>Bonjour,</p>
          <p>Veuillez trouver ci-joint les informations concernant l'élève <strong>${student?.firstName} ${student?.lastName}</strong>.</p>
          ${type === 'RECEIPT' && payment ? `<div style="background: #ffffff; padding: 12px; border-left: 4px solid #0284c7; margin: 12px 0;"><p><strong>Reçu #${payment.reference}</strong></p><p>Montant réglé : <strong>${payment.amount.toLocaleString()} FCFA</strong></p><p>Date : ${payment.date}</p></div>` : ''}
          <p style="margin-top: 20px;">Cordialement,<br/><strong>${settings.directorName || 'L\'Administration'}</strong></p>
        </div>
      `;
      ...
      const res = await fetch('/api/email/send', {
        ...
        body: JSON.stringify({ toEmail: emailForm.email, subject, htmlContent }),
      });
      const data = await res.json();
      if (data.success) {
        showToast(`E-mail envoyé avec succès via Brevo !`);
      } else {
        showToast(`Note : ${data.error || 'Erreur Brevo'}`);
      }
    } catch {
      showToast(`Document envoyé en simulation.`);
    } finally {
